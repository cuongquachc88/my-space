...
describe('nextBillingDate', () => {
  ...
  it('weekly: returns a date in the next 7 days', () => {
  ...
  it('future start date returns start date as-is (monthly)', () => {
  ...
  it('future start date returns start date (yearly)', () => {
    const future = new Date()
    future.setFullYear(future.getFullYear() + 2)
    const isoFuture = future.toISOString().slice(0, 10)
    expect(nextBillingDate(isoFuture, 'yearly')).toBe(isoFuture)
  })
})
