...
describe('generatePassword', () => {
  ...
  it('throws when length is less than number of enabled sets', () => {
    expect(() => generatePassword({ length: 3, upper: true, lower: true, digits: true, symbols: true })).toThrow('length')
  })
  ...
  it('two calls produce different results (randomness)', () => {
    const a = generatePassword({ length: 32, upper: true, lower: true, digits: true, symbols: false })
    const b = generatePassword({ length: 32, upper: true, lower: true, digits: true, symbols: false })
    expect(a).not.toBe(b)
  })
})
